...
# SW_BAYES_PROBIT
for selc in SW_ACQ_RHO #UCB RANDOM ACQ_SPEAR ACQ_RHO ACQ_EPIC ACQ_LL MUTUAL_INFO SGV_UCB #BAYES_INFO_GAIN_PROBIT #BAYES_INFO_GAIN_999 #ACQ_LL ACQ_SPEAR MUTUAL_INFO SGV_UCB UCB 
...
do
    ...
    do
        for hyper_sel in no
        do
            for model in linear gp
            do
                for user in human_choice
                do
                    for v in 1.0 0.5 0.1 400.0
                    do
