...
def main():
    ...
    model_parameters = np.empty((len(p_abss), len(envs), len(trainitrs), 20, 2), dtype=float)
        ...
        for i_env, env in enumerate(envs):
            for i_itr, trainitr in enumerate(trainitrs):
                ...
                for run in range(20):
                    ...
                    model_parameters[i_p, i_env, i_itr, run] = model.get_hyper()
    ...
    np.savez('results_hyper2'+'.npz', model_parameters=model_parameters)
